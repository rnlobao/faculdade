...
data Shape = Square Float | Rectangle Float Float
            | Triangle Float Float | Circle Float
...
list2Tree [] = Leaf
list2Tree xs = foldr insert Leaf xs
...
\begin{code}
testData :: [Client]
testData = [("Carlos","Andre",True),
            ("Joao","Silva",False),
            ("Martins","Rodrigo",False),
            ("Paulo","Correa",True)]

type Name = String
type Surname = String
type SendOffer = Bool
type Client = (String, String, Bool)

sendOffer :: [Client] -> [(String,String)]   
sendOffer [] = []
sendOffer ((x,y,z):xs)
  |z = (x,y) : sendOffer xs
  |otherwise = sendOffer xs
\end{code}